...
export default function Base64Widget() {
    const { input, setInput, mode, setMode, output, isError, copy, copied, swap, clear } = useBase64();
    ...
    return (
        <div>
            ...
            <div style={{ marginBottom: 8 }}>
                ...
                <textarea value={input} onChange={e => setInput(e.target.value)} placeholder={mode === 'encode' ? 'Enter text to encode…' : 'Enter Base64 string to decode…'} style={ta}
            ...
            <div style={{ display: 'flex', justifyContent: 'center', margin: '4px 0' }}>
                <button onClick={swap} title="Swap encode ↔ decode" style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '6px 14px', background: 'var(--white)', border: '1.5px solid var(--border)', borderRadius: 'var(--r-m)', fontSize: 12, fontWeight: 600, color: 'var(--ink-3)', cursor: 'pointer' }}>
                    <IcoSwap /> Swap
                </button>
            </div>
            ...
            <div style={{ marginBottom: 12 }}>
                ...
                <div style={{ minHeight: 80, padding: '12px', background: output ? 'var(--white)' : 'var(--page-bg)', border: `1.5px solid ${isError ? 'var(--red,#dc2626)' : output ? 'var(--border-md)' : 'var(--border)'}`, borderRadius: 'var(--r-m)', fontFamily: 'JetBrains Mono, monospace', fontSize: 13, lineHeight: 1.6, color: isError ? 'var(--red,#dc2626)' : 'var(--ink)', wordBreak: 'break-all', whiteSpace: 'pre-wrap' }}>
